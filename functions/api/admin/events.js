  ...
  await env.DB.prepare(
    `INSERT INTO events
       (id, title, date, time, place, category, tone, price,
        stripe_url, facebook_url, blurb, detail, note, image, event_date, hidden, sort_order, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, datetime('now'))`
  )
    .bind(
      e.id, e.title, e.date, e.time, e.place, e.category, e.tone, e.price,
      e.stripe_url, e.facebook_url, e.blurb, e.detail, e.note, e.image, e.event_date, e.hidden, e.sort_order
    )
    .run();
